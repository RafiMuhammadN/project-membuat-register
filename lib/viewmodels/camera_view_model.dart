...
class CameraViewModel extends BaseModel{
  ...
  void getAvailableCamera(bool mounted){
    availableCameras().then((availableCameras) {
      ...
      if (cameras.length > 0){
        ...
        _initCameraControl(cameras[selectedCameraIdx],mounted).then((void
        v) {});
      } else {
  ...
  void onCapturePressed(context) async {
    try {
      ...
      final fileName = '${dateFormat.toString()}';
  ...
  void onSwitchedCamera(bool mounted){
    ...
    _initCameraControl(selectedCamera, mounted);
  }
